...
An R package for accessing data published by the [European Central
Bank](https://www.ecb.europa.eu/) (ECB) via the [ECB Data Portal
API](https://data.ecb.europa.eu).

## What is the ECB?

The European Central Bank is the central bank for the euro area. It sets
monetary policy for the 20 countries that use the euro - most
importantly, the key interest rates that determine the cost of borrowing
across the eurozone. It also publishes a vast quantity of statistical
data covering exchange rates, inflation, money supply, lending
conditions, and government bond yields.

All of this data is freely available through the ECB’s Statistical Data
Warehouse. The problem is that accessing it programmatically requires
knowledge of SDMX series keys - opaque strings like
`FM/D.U2.EUR.4F.KR.MRR_RT.LEV` that encode dataflow, frequency, area,
currency, and instrument dimensions. This package abstracts that away
entirely.
...
The existing [`ecb`](https://cran.r-project.org/package=ecb) package
provides a general-purpose interface to the same ECB data, and works
well if you already know the SDMX series key you need. `readecb`
complements it by providing named convenience functions for the most
commonly used datasets - so you can call
...
without needing to look up key structures. It also caches results
locally and uses a lightweight dependency stack (httr2 + cli).
...
|-------------------|--------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------|
| Interest rates    | [`ecb_policy_rates()`](https://charlescoverdale.github.io/readecb/reference/ecb_policy_rates.md)       | Main refinancing rate, deposit facility rate, and marginal lending rate (daily)       |
| Interest rates    | `ecb_euribor(tenor)`                                                                                   | EURIBOR interbank lending rates at 1M, 3M, 6M, or 12M tenors (monthly)                |
| Interest rates    | [`ecb_estr()`](https://charlescoverdale.github.io/readecb/reference/ecb_estr.md)                       | Euro short-term rate - the ECB’s benchmark overnight rate that replaced EONIA (daily) |
...
| Exchange rates    | [`list_exchange_rates()`](https://charlescoverdale.github.io/readecb/reference/list_exchange_rates.md) | Available currency codes (no download needed)                                         |
| Macro             | [`ecb_gdp()`](https://charlescoverdale.github.io/readecb/reference/ecb_gdp.md)                         | Quarterly euro area real GDP in EUR millions (chain-linked volumes)                   |
| Macro             | [`ecb_unemployment()`](https://charlescoverdale.github.io/readecb/reference/ecb_unemployment.md)       | Monthly euro area harmonised unemployment rate (%)                                    |
| Macro             | [`ecb_government_debt()`](https://charlescoverdale.github.io/readecb/reference/ecb_government_debt.md) | Annual government debt-to-GDP ratio (%)                                               |
...
| Generic           | [`list_ecb_dataflows()`](https://charlescoverdale.github.io/readecb/reference/list_ecb_dataflows.md)   | All available dataflows (~100)                                                        |
| Cache             | [`clear_cache()`](https://charlescoverdale.github.io/readecb/reference/clear_cache.md)                 | Deletes all locally cached ECB files                                                  |

All download functions accept `from` and `to` date parameters and
...
The named functions above cover the most popular datasets, but the ECB
publishes over 100 dataflows. If you need something more niche - such as
payment statistics, securities issuance, or bank lending surveys - you
can use
[`ecb_get()`](https://charlescoverdale.github.io/readecb/reference/ecb_get.md)
with the dataflow name and SDMX series key. Use
...
Data is cached locally in `tools::R_user_dir("readecb", "cache")` on
...
This package is part of a family of R packages for economic and fiscal
data. They share a consistent interface - named functions, tidy data
frames, local caching - and are designed to work together.
...
<https://github.com/charlescoverdale/readecb/issues>.

## Keywords

European Central Bank, ECB, euro area, interest rates, EURIBOR, ESTR,
HICP, inflation, exchange rates, yield curve, money supply, GDP,
unemployment, government debt, monetary policy, R package
